...
def assign_rand_catalysis(molecules, RandP_sets): #Changed the argument name from "reactions" to "RandP_sets", as it will now accept the list of tuples of sets produced by FindReactionProducts!
	...
	print "\n"
	catalysations = []
	threshold = 1.0 - float(cat_probability)
...
def ReduceToFgenerated(final_closure, RAset, nonReducedSet):
	...
	for reactiontuple in copy_RAset:
...
def Retrieve_RAF_reactionNum_and_cat(final_closure, RAFset, cat_pairs, nonReducedSet):
	print "\n"
	print "###############  RETRIEVING REACTION NUMBER AND CATALYSTS...  ###################"
	print "\n"
	catalysed_RAFset = []
	for RAF_reaction in RAFset:
		for pair in cat_pairs:
			if RAF_reaction == pair[1]:
				reduced_cat_pair = copy.deepcopy(pair)				#pair.append(nonReducedSet.index(RAF_reaction)) #Add the original reaction number to the list containing catalyst(s) + reactants&products
				...
				print "catalyst list:", pair[0]					#TESTING TO ELIMINATE CATALYSTS THAT ARE NOT PART OF CLOSURE!!! IS THIS CORRECT?? To go back to previous setting 													eliminate copy of "pair" and after "RAF_reaction == pair[1]: statement just leave the two pair.append statements
				for catalyst in pair[0]:
					print "catalyst:", catalyst				
					if not catalyst in final_closure:
						print "removed catalyst:", catalyst	
						reduced_cat_pair[0].remove(catalyst)

				if reduced_cat_pair[0]:		#only append the reaction if the catalyst list is NOT EMPTY!
					reduced_cat_pair.append(nonReducedSet.index(RAF_reaction)) #Add the original reaction number to the list containing catalyst(s) + reactants&products
					catalysed_RAFset.append(reduced_cat_pair) #Add the new list containing the catalyst ID [0], the reactants and products sets [1], and the reaction number [2]
				
	print "-----------------------------------"
	if not catalysed_RAFset:
		print "There is no RAF set within this chemical network!"
	else:
...
maxRAF = Retrieve_RAF_reactionNum_and_cat(closure, myRAFset, catalysations, RPsetsAGAIN)
