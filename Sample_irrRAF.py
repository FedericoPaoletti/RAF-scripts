...
random.seed("Federico seed5") #Uncomment this in order to obtain a fixed series of random numbers! Useful for debugging.
...
def ReduceToRA(RandPsets, catalyses, foodset): # Eliminates all reactions that are not catalysed from the set "RandPsets"
	...
	while True:
		...
		print "CATALYSES LIST:", catalyses
		print "COPY CATALYSES LIST:", copy_catalyses

		for react in catalyses:
			print "Reducing to reflexively autocatalytic... Is reaction %d catalysed by molecules within the closure?" % RandPsets.index(react[1])
			
			print "catalyst list:", react[0]
...
#According to Hordjik et al. (2011) and Steel et al. (2013), a set of reactions "R'" is reflexively-autocatalytic if for all "r" in "R'", there is at least one molecule "x", element of closure[of FOOD with respect to "R'"] such that ('x', 'r') is part of the set of catalysations...
			...
			#print "copy catalyst list:", copy_catalyses[RandPsets.index(react)][0]

			catalysis_counter = 0

			if not react[0]: #If the list of catalysts of the reaction "react" is empty, then remove the reaction!
				print "No! Eliminated the following reaction from the RP list and from the catalysation list:"
				print react
				copy_RandPsets.remove(react[1])
				copy_catalyses.remove(react)
				...
				for mol in react[0]:
					...
					if mol in closure:
						...
						#react[0].remove(mol)
						#if react_copy in copy_catalyses:
						#	copy_catalyses[copy_catalyses.index(react_copy)][0] = react[0] #remove the catalyst that is not within the closure set!
						if react in copy_catalyses:
							copy_catalyses[copy_catalyses.index(react)][0].remove(mol)
		...
		if len(RandPsets) == len(copy_RandPsets) and all([element[0] for element in copy_catalyses]): #the all statement returns TRUE if all catalyst lists are non-empty!
			...
			RandPsets = copy.deepcopy(copy_RandPsets)
			catalyses = copy.deepcopy(copy_catalyses)
		"""
		mike += 1
		if mike > 200:
			break
	...
	print "------------------------------------------------"
	return copy.deepcopy(copy_RandPsets), copy.deepcopy(copy_catalyses) # --> The reduced reaction set and catalysation set!
...
def ComputeClosure(Food, RAset):
	...
	while True:
		...
		for reactiontuple in RAset:
			print "REACTION TUPLES:", reactiontuple[0], reactiontuple[1]
...
def modify_toyChem():
	with open("myRAF.txt", 'r') as RAF_dotfile, open("RAFhighlight.txt", 'w') as finalfile, open("network_output1.txt", 'r') as toyChem_dotfile:
		for line in toyChem_dotfile: #Stoichiometry is maintained due to order of for loops!!!
			print "writing RAFhighlight.txt file..."
			match_flag = 0
			RAF_dotfile.seek(0)
...
def RAF_algorithm(myfood, myRPlist, myRPlist_2,  mycatalysations):
	
	RA_set = ReduceToRA(myRPlist, mycatalysations, myfood)

	RA_reactions = RA_set[0]
	RA_catalysations = RA_set[1]

	
	finalclosure = ComputeClosure(myfood, RA_reactions)
	RAF = ReduceToFgenerated(finalclosure, RA_reactions, RA_catalysations, myRPlist_2)

	#RAF_only_reaction_sets = [element[1] for element in RAF]
	#print "Within RAF algorithm... RAFset:", RAF
	return RAF

def irrRAFsample(RAFset): #ASK ALEX POPINGA IF SHE CAN GET THIS SAMPLING ALGORITHM TO WORK!!!
	print "\n"
	print "################ SAMPLING AN IRREDUCIBLE RAF FROM THE MAXRAF...##################"
	print "\n"
	#print "Within irrRAFsample function before shuffling.. RAFset:", RAFset
	random.shuffle(RAFset)
	#print "Within irrRAFsample function after shuffling.. RAFset:", RAFset
	full_set = copy.deepcopy(RAFset)
	print "copy RAF set:", full_set
	print "RAF set:", RAFset

	mike = 0
	while True:
		for reaction in full_set:
		
#current_RPlist = [element[1] for element in RAFset]

	#if reaction in current_RPlist:

			print "reaction:", reaction
			working_set = copy.deepcopy(RAFset)

			if reaction in RAFset: #ADDED THIS LINEEE!!!

				RAFset.remove(reaction)

				print "Removed the following reaction:", reaction

				reaction_products = [element[1] for element in RAFset]
			#reaction_products2 = [element[1] for element in RAFset]

				reduced_RAF = RAF_algorithm(food, reaction_products, RPlist_2, RAFset) 
	...
	print "An irreducible RAF from this maxRAF is:", RAFset	
	return RAFset
...
maxRAF = RAF_algorithm(food, RPlist, RPlist_2, catalysations)

irrRAF = irrRAFsample(maxRAF)

if irrRAF:
	numberedRAF = Retrieve_RAF_reactionNum(irrRAF, RPlist_2) 
